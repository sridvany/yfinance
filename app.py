...
st.caption("Varlık sembolünü bilmiyorsanız Gemini'ye 'yfinance ...... tickerı nedir' yazın. Birden fazla sembol için virgülle ayırın (örn: THYAO.IS, AAPL, BTC-USD).")

# --- 1. Sembol Girişi ---
symbol_input = st.text_input("Sembol(ler)", placeholder="Örn: THYAO.IS, AAPL, BTC-USD")
...
if symbol_input:
    ...
    for symbol in symbols:
        ...
        with col1:
            start_date = st.date_input("Başlangıç", value=oldest_date, min_value=oldest_date, max_value=newest_date, key=f"start_{symbol}")
        with col2:
            end_date = st.date_input("Bitiş", value=newest_date, min_value=oldest_date, max_value=newest_date, key=f"end_{symbol}")
        ...
        for row in rows:
            ...
            for i, col_name in enumerate(row):
                with checkbox_cols[i]:
                    if st.checkbox(col_name, value=True, key=f"cb_{symbol}_{col_name}"):
